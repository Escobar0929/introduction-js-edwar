// Arrow fuction
// En JS es una foma mas corta de escribir funciones usando la sintaxis mas breve 

// Sintaxis mas brveev, ideal para funciones pequeñas
// NO tiene su propio this, hereda el this del contexto donde se define


const add = function(){
    console.log(16 + 3)
}

add(); 

// Funcion con parametor 
const add2 = function(num1, num2){
    console.log(num1 + num2);
}

add2(4, 8)

// Las llaves son opcionales cuando despues de la flecha solo tengo una sentencia 
const add3 = (num3, num4) => console.log(num3 + num4);
add3(6, 9);

//================================

// Otro Ejemplo 
const learn = (technology) => {
    console.log('Aprendiendo ${technology}');
}

learn ('JavaScrip');

// =======================================

const months = ["May", "January", "July", "June", "December", "November", " February"];

// Este no tiene constructor
const shoppingCart = [
    {prductName:'Smatr TV 60', price: 900},
    {prductName:'Smatr TV 50', price: 700},
    {prductName:'Tablet', price: 500},
    {prductName:'Smart phone', price: 900},
    {prductName:'Google assistand', price: 600},
    {prductName:'Sound-bar', price: 400},
    {prductName:'Play station', price: 800},
    {prductName:'Wii', price: 450},
    {prductName:'Mause', price: 50},
]

// moths.forEach( function(month){
//     if(month == 'November' || moth == 'December'){
//         console.log('Se verifico en el arreglo y el mes Si exixte')
//     }else{
//         console.log('El mes No existe')
//     }
// })

//============================

months.forEach(month =>{
    if(month == 'November' ||month == 'December'){
        console.log('Se verifico en el arreglo y el mes Si exixte')
    }else{
        console.log('El mes No existe')
    }
})

// const result = shoppingCart.some(function(produc){
//     return produc.price === 800
// })

// console.log(result);

const result = shoppingCart.some(produc => produc.price === 800)

console.log(result);

//==============================
// let an const: new ways of declaring variables
// Arrow fuction: () => {}.

// const result3 = shoppingCart.reduce(function(total, product){
//     return total + product.price;
// }, 0)

// console.log(result3)

const result3 = shoppingCart.reduce ((total, product) => total + product.price, 0)

console.log(result3)

//==================================

// const result6 = shoppingCart.filter(Function(product){
//     return product.price < 600
// })
// console.log(result6);

const result6 = shoppingCart.filter(product => product.price < 600)
console.log(result6)

//===========================

// function greet (userName){
//     console.log('Hello ${userName}');
// }
// console.log(greet("Brahian"))

// greet = userName => `Hello ${userName}`

// console.log(greet("Brahian")) 

// ===================================

// const boost = num =>num + num;
// console.log(boost(4));

// const boost2 = num => Math.pow(num,2);
// console,log(boost2(2));

// ==============================

// Arrow fuction con retorno explicito 

// Se usa cuando el cuerpo de una funcion tiene llaves{}

const add10 = (num4, num5) => {
    const result7 = num4 + num5;
    return result7
}

console.log (add10(15, 7))

// ========================

// Retorno Implicito, cuando la funcion No tiene llaves realiza el retorno implicitamente 
const add11 = (num6, num7) => {console.log(num6 + num7); num6 + num7}

console.log(add11(4, 6))